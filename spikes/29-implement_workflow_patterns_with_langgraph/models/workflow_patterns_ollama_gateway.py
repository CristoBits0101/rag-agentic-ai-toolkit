...
def select_best_available_ollama_model(installed_models: list[str] | None = None) -> str:
    ...
    for preferred_model in OLLAMA_MODEL_CANDIDATES:
        ...
        for installed_name in installed_models:
            if normalize_model_name(installed_name).split(":", maxsplit=1)[0] == preferred_base:
                return installed_name
    return installed_models[0]


def build_workflow_patterns_model(model_name: str | None = None, tags_loader=list_installed_ollama_models):
    if ChatOllama is None:
        raise RuntimeError("LangChain Ollama is not available. Install langchain-ollama before running practica 29.")
    selected_model = model_name or select_best_available_ollama_model(tags_loader())
    return ChatOllama(model=selected_model, temperature=0, validate_model_on_init=False)