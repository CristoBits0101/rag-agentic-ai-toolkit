...
from config.reflection_agent_config import OLLAMA_MODEL_CANDIDATES
from config.reflection_agent_config import OLLAMA_TAGS_URL
...
def build_reflection_agent_ollama_chat_model(
    model_name: str | None = None,
    tags_loader=list_installed_ollama_models,
):
    if ChatOllama is None:
        raise RuntimeError(
            "LangChain Ollama is not available. Install langchain-ollama before running practica 25."
        )
