...
1. Compilacion: `python -m compileall spikes\35-building_agentic_ai_systems_with_the_beeai_framework`.
2. Practica: `venv\Scripts\python.exe .\spikes\35-building_agentic_ai_systems_with_the_beeai_framework\main.py`.
3. Tests: `venv\Scripts\python.exe -m pytest -c NUL --rootdir . tests\unit\test_spike_35_building_agentic_ai_systems_with_the_beeai_framework.py`.
...
6. `WikipediaTool` `ThinkTool` y `OpenMeteoTool`: Enseñan herramientas y razonamiento explicito.
7. `ConditionalRequirement` y `AskPermissionRequirement`: Muestran control declarativo y human in the loop.
8. `SimpleCalculatorTool`: Enseña creacion de herramientas custom.
