    ...
    def kickoff(self, inputs: dict[str, Any]):
        ...
        for index, task in enumerate(self.tasks):
            ...
            if task.executor is None:
                ...
                raw_result = task.executor(task.agent, description, context_text, inputs)

            if task.output_pydantic and not isinstance(raw_result, str):
                rendered = json.dumps(raw_result.model_dump(), indent=2, ensure_ascii=False)
            else:
