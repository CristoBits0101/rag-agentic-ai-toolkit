...
from config.youtube_tool_calling_agent_config import OLLAMA_MODEL_CANDIDATES
from config.youtube_tool_calling_agent_config import OLLAMA_TAGS_URL
...
def build_youtube_tool_calling_ollama_chat_model(
    model_name: str | None = None,
    tags_loader=list_installed_ollama_models,
):
    if ChatOllama is None:
        raise RuntimeError(
            "LangChain Ollama is not available. Install langchain-ollama before running practica 21."
        )
