...
| 14 | Vision Multimodal Basica. | Construye mensajes con imagen y texto responde preguntas visuales hace captioning en lote matching simple contra un catalogo y una extension `Style Finder` con `Gradio` retrieval visual y analisis de moda. Tambien incluye variantes reales con `llava` `llama3.2-vision` y `qwen2.5vl` en `Ollama`. | A entender el patron basico de `vision querying` `VQA` `Image Captioning` y como escalarlo a una app completa de `multimodal RAG` para moda. | [14-basic_vision_multimodal_lab](./14-basic_vision_multimodal_lab/README.md) |
...
5. Cierra con `11` `12` `13` `14` `15` y `16` para trabajar `FAISS` contenido multimedia `Text to Speech` vision multimodal captioning visual apps `Gradio` de moda asistentes de reunion y generacion de imagenes.
