...
| 14 | Vision Multimodal Basica. | Construye mensajes con imagen y texto responde preguntas visuales hace captioning en lote y matching simple contra un catalogo. Tambien incluye variantes reales con `llava` `llama3.2-vision` y `qwen2.5vl` en `Ollama`. | A entender el patron basico de `vision querying` `VQA` `Image Captioning` y similitud visual ligera con demo local o modelos reales. | [14-basic_vision_multimodal_lab](./14-basic_vision_multimodal_lab/README.md) |
...
5. Cierra con `11` `12` `13` `14` `15` y `16` para trabajar `FAISS` contenido multimedia `Text to Speech` vision multimodal captioning visual asistentes de reunion y generacion de imagenes.
